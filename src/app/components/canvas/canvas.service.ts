...
import { dtr, Sphere, world, view, camera } from './sphere';
...
/**
 * Should to be 'function' can't be arrow function!!!
 * @param minTime
 * @param maxTime
 */
const closeValue = function (minTime, maxTime) {
    this.flag = 0;
    this.progress = 0;
    this.startTime = 0;
    this.durationTime = 0;
...
export class CanvasService {
    ...
    private sphereNum = 20; // number of letters
    private strokeColor = 'rgba(255,255,255,0.1)';
    ...
    private bufferImages: any = {};
    private vibrateCV = new closeValue(200, 500);
    private invertCV = new closeValue(1000, 1200);
    private canvas;
    private ctx;
    private s = [];
    vibrateFlag = false;
    ...
    _init(mainCanvas: ElementRef, bufferCanvases: QueryList<ElementRef>) {
        let charsLength = 0;
        let charCounter = 0;
        for (let i in chars) {
            charsLength++;
            this.bufferImages[i] = new Image();
            this.bufferImages[i].src = chars[i];
            this.bufferImages[i].onload = () => {
                charCounter++;
                if (charCounter === charsLength) {
                    this.bufferDraw(mainCanvas, bufferCanvases);
                }
            };
        }
    ...
    bufferDraw(mainCanvas: ElementRef, bufferCanvases: QueryList<ElementRef>) {
        this.canvas = mainCanvas.nativeElement;
    ...
    setup() {
        for (let i = 0; i < this.sphereNum; i++) {
            this.s[i] = new Sphere({
                radius: 50,
                particleNum: 150,
                center: {x: 0, y: 0, z: 0} // 70 * i - (this.sphereNum - 1) * 70 / 2
            }, this.ctx, this.vibrateFlag);
        }
    }
    ...
    textSetChanger() {
        let timeOut = 100;
        let recurtionRun = (_timeOut = timeOut) => {
            setTimeout(() => {
                this.textChanger(
                ...
                recurtionRun(this.textSet[this.textSetChangerIncrement].time);
            }, _timeOut);
        };
